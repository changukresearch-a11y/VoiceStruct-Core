...
| **`결과값_필드_레퍼런스.html`** | 개발자 | 필드 데이터 사전 — 공시18·뉴스24 필드별 출처·산출식·등급 경계 (※뉴스28→24 문서 갱신 예정) |
...
공시·뉴스는 **완전 별개 2객체**로 넘어간다(합치기는 Strategist). 정식 전달 = **DB 스냅샷 테이블** `tb_disclosure`(18)·`tb_news`(24), **PK=(ticker, collected_at)** 시계열 append → Strategist는 종목별 **최신 행**을 JOIN해 읽음. 보조 = `to_prompt()`·JSON. (스케줄러 `--bundles`로 매 사이클 종목당 1행 자동 append, 신호 없으면 has_signal=0)
모든 점수는 **0~1 소수 2자리**, 자유서술(summary·reason)·라벨은 **영어**(전략가 에이전트가 읽음). (회사명·카테고리는 tb_universe JOIN)

- **핵심 0~1 지표**: `sentiment_score`(0=악재·0.5=중립·1=호재, 호재만 신뢰 감쇠) · `importance`(방향 무관 강도) · `risk_score` · `confidence` · `source_trust`(뉴스) · `grade_score`(뉴스)
- **원문 식별**: 공시 `filing_title·filing_no·filed_at`(초 단위) / 뉴스 `news_title·source·published_at`(초 단위) · 뉴스 `top_evidence`(정확히 3개, 부족분 null)
- **부가**: `reason`(근거 한 줄) · `summary`(3~4줄) · `keywords`(5개) · `hard_block` · `collected_at`
  - ※ 2026-07-06 팀 명세: 공시 24→18·뉴스 31→28(삭제 `company_name·category·verdict`+공시 `confirmed_score·fact_check·ref`).
  - ※ 2026-07-08 팀 명세: **뉴스 28→24** — 루머/팩트 이진판별 4필드 삭제(`news_confirmed·news_rumor·confirmed_score·fact_check`), `top_evidence` 3고정.
...
# --bundles: 매 사이클 종목당 tb_disclosure/tb_news에 집계 1행 append(신호 없으면 has_signal=0)
python run_scheduler.py --once --limit 50 --forms 8-K,10-Q --news --llm --save --bundles
...
- **운영**: 유니버스 시총순위 확장(2000) · 메타레벨 증분 스케줄러 · **배치 번들 스냅샷(`--bundles`: 매 사이클 종목당 tb_disclosure/tb_news 1행 append, 공시 필드별 집계)** · 전방수익률 백테스트(측정만)
- **Strategist 계약**: 별개 2객체(공시18·뉴스24) · 0~1 통일 · 요약/키워드/판정 · 영어화 · 원문 식별 · top_evidence 3고정
