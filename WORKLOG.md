...
## 0.5 최근 변경 이력 (2026-07-02 ~ 07-08)

### 2026-07-08 — 뉴스 번들 24필드 축소 (루머/팩트 이진판별 4필드 삭제 + top_evidence 3고정)

명세 §06 최종본(07-08 결정) 반영. "확정이냐 루머냐를 LLM이 무 자르듯 판별하기 어렵다"는 팀 결정으로
**NewsBundle 28→24**. 신뢰도는 source_trust(LLM)·grade_score(코드 도메인)·news_count·top_evidence로 커버.

- **삭제 4필드**: `news_confirmed`·`news_rumor`·`confirmed_score`·`fact_check` — `analysis_bundle.py`
  NewsBundle 스키마·build_news_bundle 대입부·`_fact_check_news` 헬퍼·to_prompt(confirmed/rumor·Fact-check 줄) /
  `db.py` tb_news DDL·`_TB_NEWS_COLS`(24컬럼). `is_confirmed`는 **기사별 내부 가중(_news_conf·_news_credible)에는
  유지**(출력만 제거, 리스크 낮음 — 표본 쌓이면 재검토).
- **top_evidence 1→3 고정**: 구 `scored[:1]`(1개) → `scored[:3]` + None 패딩 → **항상 정확히 3칸**(부족분 None,
  명세 §06). 타입 `list[str|None]`. to_prompt·`_bundle_row`(DB fold)에서 None 가드.
- **DB 비파괴**: CREATE IF NOT EXISTS라 기존 DB의 옛 tb_news는 4개 죽은 컬럼 NULL로 남되 INSERT는 24컬럼으로 정상
  (새 DB는 깔끔히 24컬럼). 기존 데이터 안 지움.
- 검증(오프라인·임시DB): 필드셋 24(삭제 4 부재)·공시 18 불변·top_evidence 1건→3칸 None패딩·5건→상위3·to_prompt
  이진판별 표기 제거·JSON 24키·DB 라운드트립 24컬럼·빈번들 has_signal=0 전부 통과. 배치 배선 재검증·전 엔트리
  import OK(삭제필드 잔재=docstring 설명뿐).
- **미결(문서 현행화)**: `Document/`의 데이터스키마명세_v2(md·html)·결과값_필드_레퍼런스·뉴스_상세설계서·
  정보분석_개발보고서가 아직 28필드 기준 → 24필드로 갱신 필요(별도).

### 2026-07-08 — 배치 번들 스냅샷 파이프라인 연결 (tb_disclosure/tb_news 자동 append)

명세 §05·§06의 "매 사이클 50종목 → 50행 append"(신호 없으면 has_signal=0)를 실제 스케줄러에 배선.
그동안 tb_* 스냅샷 저장은 `run_bundle.py`(단건)에만 있었고 스케줄러는 per-signal 로그(disclosure_signals/
news_signals)만 남겨, "종목당 집계 1행 시계열 누적"이 자동화돼 있지 않았음.

- **신규** `app/scheduler/bundle_snapshot.py` — `BundleAccumulator`: 종목별 오늘 분석결과를 프로세스
  메모리에 누적(공시·뉴스 모두 키맵으로 전부 보관) → 매 사이클 그 전체로 집계 번들 생성.
  `roll(trade_date)`로 거래일 바뀌면 초기화. **공시 필드별 집계(명세 §05 "한 행으로 모으되
  hard_block=1·risk_score 최댓값 보존")**: 방향성(event_type·sentiment·importance)·원문 메타는 대표
  1건(파산·상폐 hard_block 우선→importance 최대, `disclosure_rep`), **hard_block=OR(`disclosure_block`)·
  risk_score=최댓값(`disclosure_risk_max`)**을 별도 보존해 저장 시 override(치명 위험이 대표 아니어도 안 묻힘).
  뉴스는 `build_news_bundle`이 이미 신뢰가중 필드별 집계.
- **수정** `app/scheduler/scheduler.py` — `run_cycle`에 `save_bundles`·`bundle_acc` 추가. 한 수집 순회에서
  공시 "new"는 누적기에 add_disclosure, 뉴스는 `_process_news`가 add_news, 종목마다 `_save_ticker_bundles`로
  tb_disclosure/tb_news에 1행 append(신호 없어도 has_signal=0). 공시 저장 시 대표 번들에 risk_max·hard_block
  override(필드별 집계). **이중 주기 반영**: tb_disclosure는 공시 틱(process_filings)만, tb_news는 뉴스 틱마다.
  `run_forever`는 누적기를 프로세스 수명 동안 유지(사이클 간 누적).
- **수정** `run_scheduler.py` — `--bundles` 플래그(save_bundles). per-signal 저장(`--save`)과 독립.
- **수집 이중 안 함**: 기존 순회 결과를 재사용해 번들 생성(SEC/RSS 재호출 없음). per-signal 경로 무손상.
- 검증(오프라인·임시DB·네트워크 패치): **공시 필드별 집계**(대표 imp8이어도 risk_score=오늘 max 0.8 보존·
  hard_block=OR) · 3종목→각 3행(has_signal=0) · 2사이클 append(고유 collected_at 6개) · 뉴스 has_signal=1 ·
  롤오버 초기화 전부 통과. import·argparse OK.
- **한계(문서화)**: 누적이 프로세스 메모리라 **재시작 시 초기화** → 재시작 전 분석분은 새 신호 뜨기 전까지
  번들에서 빠질 수 있음(기존 스케줄러 seen-집합 dedup과 같은 성격). 완전 복원(재시작 시 *_signals에서
  오늘분 로드)은 후속. `--once`는 그 사이클에 새로 처리된 공시만 has_signal=1(steady-state는 run_forever).
- 실행: `python run_scheduler.py --news --limit 50 --forms 8-K,10-Q --llm --save --bundles`

### 2026-07-06 — 팀 명세 반영: 번들 필드 정리 + 스냅샷 DB 계약

팀장(김지현) 전체 파이프라인 명세(`Quantinue 1차 — 파이프라인 흐름순 상세 명세 · 팀 여름이었다.html`,
05공시/06뉴스가 우리 파트)의 필드 정리 제안을 **협의 수용**해 구현.

- **번들 필드 축소**: `DisclosureBundle` 24→**18**(삭제 `company_name·category·confirmed_score·fact_check·verdict·ref`),
  `NewsBundle` 31→**28**(삭제 `company_name·category·verdict`; `source_trust·grade_score·confirmed_score·fact_check·top_evidence·peak_importance·ref`는 유지).
- **개명·재배치**: `as_of`→`trade_date`, `created_at`→`collected_at`, `reason`을 `sentiment_score` 바로 아래로.
- **전달 구조 변경**: 2객체 직접전달 → **DB 스냅샷** `tb_disclosure`(18)·`tb_news`(28),
  **PK=(ticker, collected_at)** 시계열 append, Strategist는 종목별 최신 행 JOIN.
  `save_disclosure_bundle`/`save_news_bundle`/`latest_disclosure`/`latest_news` 신규.
  기존 `disclosure_signals`/`news_signals`(per-signal 로그, 스케줄러·백테스트용)는 **그대로 유지** — tb_*는 별개.
- 반영: `app/common/analysis_bundle.py`·`app/storage/db.py`·`run_bundle.py`(--save, --category 제거)·`run_screening_input.py`.
- 문서 갱신: `데이터스키마_인터페이스명세_v2`(md+html)·`결과값_필드_레퍼런스`·`공시/뉴스_상세설계서`·
  `정보분석_개발보고서`·`결과값_산출방식·의미·전략가활용`·README·이 파일.
- 검증: 필드 18/28·개명·재배치·to_prompt·DB 라운드트립·전체 import·기존 백테스트 무손상.
- **미결**: `verdict` 삭제는 우리가 07-03에 의도적으로 넣었던 것 → 김지현과 최종 확정 필요(명세엔 "협의 예정").
