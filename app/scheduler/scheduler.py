...
def run_cycle(limit: int = 5, forms: tuple[str, ...] = DEFAULT_FORMS,
    ...
    for ticker in get_active_tickers(limit):
        ...
        for form in forms:
            meta = metas.get(form)
            if not meta:
                report.filings.append(FilingOutcome(ticker, form, None, "no-filing"))
                continue
            report.filings.append(
...
def run_forever(interval_sec: int = 900, **cycle_kwargs) -> None:
    """interval_sec 간격으로 run_cycle을 반복. Ctrl+C로 종료.

    seen_news 집합을 사이클 간 공유해 같은 뉴스의 반복 LLM 호출을 막는다.
    """
    ...
    try:
        while True:
            cycle += 1
            report = run_cycle(seen_news=seen_news, **cycle_kwargs)
            yield_summary(cycle, report)
            time.sleep(interval_sec)
    except KeyboardInterrupt:
        print("\n⏹️  스케줄러 종료 (Ctrl+C)")


def yield_summary(cycle: int, report: CycleReport) -> None:
    ...
    print(f"[cycle {cycle} @ {report.started_at}] 공시 {n}건 "
          f"(new {report.new}·unchanged {report.unchanged}·err {report.errors})"
          f"{news_note}")
